...
const StringDecoder$2 = string_decoder.StringDecoder;
...
methods.forEach(name => {
  ...
  Agent$1.prototype[name] = function(url, fn) {
    const req = new node$1.Request(method, url);
...
Request.prototype.request = function(){
  ...
  try {
    const query = lib$1.stringify(this.qs, {
      indices: false,
...
Request.prototype._emitResponse = function(body, files) {
  const response$1 = new response(this);
  ...
  req.once('response', res => {
    ...
    if (!parser) {
      if (responseType) {
        ...
        const form = new lib.IncomingForm();
